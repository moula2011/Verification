    ...
    <section id="moula">
        ...
        <div class="medi-container absolute inset-x-12 top-28 bg-white rounded-xl overflow-hidden md:w-100">
            ...
            <div class="tableveri h-auto w-3/4 m-4 medi-client rounded-md border-red-200" style="background-color:#C9DFEC; height:678px;">
                <div class="bg-white flex flex-col m-4 medi-client rounded-md border-red-200" style="width: 1050px; height:657px; overflow: auto;">
                    <label for="" class="m-2 ml-6" style="opacity: 0.7;">
                        <b class=" text-2xl">NOT VERIFIED ACTS  </b>
                        <?php $num=0; foreach($acts as $act): if($act->verified == 0){ $num +=1;} ?>
                        <?php  endforeach?>
                        <b class=" text-2xl ml-6" style="color: red;"><?= $num?></b> ACT<?php if($num > 1){echo "S";}?>
                    </label>
                    <table class="w-90 m-2 medi-btn">
                        <thead class="bg-white ">
                            <tr class="medi-btn" style="background-color:#CCC; height: 50px;">
                                ...
                                <th class="h-10 medi-btn w-20">TA <br> MUSA</th>
                                <th class="h-10 medi-btn w-20">TYPE </th>
                                <!-- <th class="h-10 medi-btn w-20">PRIX ACTUEL	</th> -->
                                <th>Verify</th>
                            </tr>
                        </thead>
                        <?php $i=0; foreach($acts as $act):  $i++; ?>
                            <tbody class="medi-btn">
                                ...
                                ?>
                                    <td class=""><?= $i.' .'?></td>
                                    <td class=""><?= $act->description?></td>
                                    <td class="text-center"><?php if($act->insured == 1){ echo "Yes";}else{echo "NOT";}?></td>
                                    <td class="text-center">
                                        <?php if($act->verified == 0){ 
                                            echo '<label class="px-2 py-1 rounded-md text-white" style="background-color:red;">'.$act->unit_price."</label>";
                                        }else{
                                            echo $act->unit_price;
                                        }
                                        ?>
                                    ...
                                    <td class="medi-btn">
                                        <button class="p-1 px-2 m-2 medi-btn rounded-md" style=" background-color:#66CDAA ; color:whitesmoke;">+</button>
                                    </td>
                                </tr>
                            </tbody>
                        <?php endforeach ?>
