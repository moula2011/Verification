                ...
                <a href="check.php">
                    <?php foreach($consult as $check): $v_check = $check->id; endforeach; if($check->checked == 1){?>
                        <div class="medi-magic medi-magic-btn my-2 mx-2 p-1 bg-gradient-to-r bg-gray-light rounded-md">&nbsp; 
                            <b class="text-3xl text-center" id="unchecked"><?php $v_check; ?>0</b> &nbsp;Unchecked
                        </div>
                    <?php }else{?>
                                        ...
                                        <?php 
                                            if($uncheck->checked != 0){
                                                foreach($uncheck->items->medicines as $meds): 
                                                    echo '<input type="checkbox" name="item[]" value="" class="py-2 ml-2 mr-1 my-1" id="med_item">
                                                    <label for="med_item" class="w-70 text-md mt-2">';
                                                    echo $meds->med_item; 
                                                    echo '</label>';
                                                    echo '<label for="chkp" class="w-28 text-md mt-2 ml-3 ">
                                                    (<b>'.$meds->med_quantity.'</b>) (<b style="color: red;">U-P: </b><b>'.$meds->med_u_p.'</b> Rwf)
                                                </label><br>';
                                                endforeach;
                                            }else{echo 0;}
                                        ?>                                                                                
                                        ...
                                        <?php 
                                            if($uncheck->checked != 0){
                                                foreach($uncheck->items->consommables as $consoms): 
                                                    echo '<input type="checkbox" name="item[]" value="" class="py-2 ml-2 mr-1 my-1" id="chkp">
                                                    <label for="chkp" class="text-md mt-2 text-black medi_limit_span_check ">';
                                                    echo $consoms->conso_item; 
                                                    echo '</label>';
                                                    echo '<label for="chkp" class="text-md mt-2 ml-3 ">
                                                    (<b>'.$consoms->conso_quantity.'</b>) (<b style="color: red;">U-P: </b><b>'.$consoms->conso_u_p.'</b> Rwf)
                                                </label><br>';
                                                endforeach;
                                            }else{echo 0;}
                                        ?>
