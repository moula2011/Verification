...
$period=$_REQUEST['period'];
    ...
    $coveriamounted=0;$meveriamounted=0;$converiamounted=0;$laveriamounted=0;$soveriamounted=0;$hoveriamounted=0;
    $done=0;$days_left=0;$days_done_to_veri=0;
    ...
    foreach($consult as $check): 
        ...
        foreach($check->items->verification->consultation as $veri):if($check->period == $period){ $coveriamounted+=$veri->amounted;}endforeach;
        foreach($check->items->verification->medicines as $veri):if($check->period == $period){ $meveriamounted+=$veri->amounted;}endforeach;
        foreach($check->items->verification->consommables as $veri):if($check->period == $period){ $converiamounted+=$veri->amounted;}endforeach;
        foreach($check->items->verification->laboratoire as $veri):if($check->period == $period){ $laveriamounted+=$veri->amounted;}endforeach;
        foreach($check->items->verification->soins as $veri):if($check->period == $period){ $soveriamounted+=$veri->amounted;}endforeach;
        foreach($check->items->verification->hospitalisation as $veri):if($check->period == $period){ $hoveriamounted+=$veri->amounted;}endforeach;
        //============ Ambulance ==========================
